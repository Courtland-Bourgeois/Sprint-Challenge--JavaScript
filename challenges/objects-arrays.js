...
const universities = [];
graduates.forEach(uni => universities.push(uni.university));
...
const contactInfo = [];
graduates.filter(contact => contactInfo.push(`${contact.first_name} ${contact.email}`));
...
zooAnimals.map(low => lowCaseAnimalNames.push(low.animal_name.toLowerCase()))
...
*/
const lowPopulationAnimals = zooAnimals.filter(lowPop => {
...
*/
const populationTotal = zooAnimals.reduce((accum, total) => {
